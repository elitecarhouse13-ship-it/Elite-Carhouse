...
const ZONAS_COMUNES_SUGERIDAS = ["Piscina", "Gimnasio", "Salón social", "Zona BBQ", "Zona infantil", "Portería 24h", "Ascensor", "Cancha múltiple"];
...
function Inventario({ productos, solicitudes, onGuardarProducto, esAdmin, sesion, admins, onGuardarAdmin, onEliminarAdmin, comisionistas, onGuardarComisionista, onEliminarComisionista, onCambiarEstado, onEliminar, papelera, onRestaurar, onEliminarDefinitivo }) {
  ...
  const [ordenPor, setOrdenPor] = useState("reciente");
  ...
  const limpiarFiltros = () => {
    setPrecioMin(""); setPrecioMax(""); setAnioMin(""); setAnioMax(""); setKmMax(""); setOrdenPor("reciente");
  };
  ...
  const filtrados = productosPorCategoria
    ...
    .filter((p) => {
      if (precioMin !== "" && (p.precioBase ?? 0) < Number(precioMin)) return false;
      if (precioMax !== "" && (p.precioBase ?? 0) > Number(precioMax)) return false;
      if (anioMin !== "" && (p.anioModelo == null || p.anioModelo < Number(anioMin))) return false;
      if (anioMax !== "" && (p.anioModelo == null || p.anioModelo > Number(anioMax))) return false;
      if (kmMax !== "" && (p.kilometraje == null || p.kilometraje > Number(kmMax))) return false;
      return true;
    })
    .sort((a, b) => {
      if (ordenPor === "precioAsc") return (a.precioBase ?? 0) - (b.precioBase ?? 0);
      if (ordenPor === "precioDesc") return (b.precioBase ?? 0) - (a.precioBase ?? 0);
      if (ordenPor === "kmAsc") return (a.kilometraje ?? Infinity) - (b.kilometraje ?? Infinity);
      return (b.fechaPublicado ?? 0) - (a.fechaPublicado ?? 0);
    });
  ...
  return (
    <div style={styles.screen} className="app-shell">
      ...
      <div style={styles.searchRow}>
        ...
        <button style={styles.filtrosBtn} onClick={() => setFiltrosAbiertos((o) => !o)} title="Filtros">
          <SlidersHorizontal size={16} />
          {filtrosActivos && <span style={styles.filtrosBadgeDot} />}
        </button>
      </div>
      ...
      {filtrosAbiertos && (
        <div style={styles.filtrosPanel}>
          <div style={styles.filtrosGrid}>
            <div>
              <div style={styles.smallLabel}>PRECIO DESDE</div>
              <PrecioInput style={styles.input} placeholder="0" value={precioMin} onChange={setPrecioMin} />
            </div>
            <div>
              <div style={styles.smallLabel}>PRECIO HASTA</div>
              <PrecioInput style={styles.input} placeholder="Sin límite" value={precioMax} onChange={setPrecioMax} />
            </div>
            <div>
              <div style={styles.smallLabel}>AÑO DESDE</div>
              <input style={styles.input} type="number" placeholder="Ej: 2015" value={anioMin} onChange={(e) => setAnioMin(e.target.value)} />
            </div>
            <div>
              <div style={styles.smallLabel}>AÑO HASTA</div>
              <input style={styles.input} type="number" placeholder="Ej: 2024" value={anioMax} onChange={(e) => setAnioMax(e.target.value)} />
            </div>
            <div>
              <div style={styles.smallLabel}>KM MÁXIMO</div>
              <PrecioInput style={styles.input} placeholder="Sin límite" value={kmMax} onChange={setKmMax} />
            </div>
            <div>
              <div style={styles.smallLabel}>ORDENAR POR</div>
              <select style={styles.input} value={ordenPor} onChange={(e) => setOrdenPor(e.target.value)}>
                <option value="reciente">Más reciente</option>
                <option value="precioAsc">Precio: menor a mayor</option>
                <option value="precioDesc">Precio: mayor a menor</option>
                <option value="kmAsc">Kilometraje: menor a mayor</option>
              </select>
            </div>
          </div>
          <div style={styles.row8}>
            <button style={styles.cancelBtn} onClick={() => setFiltrosAbiertos(false)}>Cerrar</button>
            {filtrosActivos && (
              <button style={styles.confirmSoldBtn} onClick={limpiarFiltros}>Limpiar filtros</button>
            )}
          </div>
        </div>
      )}
...
function ProductoForm({ inicial, onCancel, onGuardar }) {
  ...
  const [areaM2, setAreaM2] = useState(inicial?.areaM2 ?? "");
  const [habitaciones, setHabitaciones] = useState(inicial?.habitaciones ?? "");
  const [banos, setBanos] = useState(inicial?.banos ?? "");
  const [parqueaderos, setParqueaderos] = useState(inicial?.parqueaderos ?? "");
  const [piso, setPiso] = useState(inicial?.piso || "");
  const [estrato, setEstrato] = useState(inicial?.estrato ?? "");
  const [administracion, setAdministracion] = useState(inicial?.administracion ?? "");
  const [libreGravamen, setLibreGravamen] = useState(inicial?.libreGravamen ?? true);
  const [gravamenEntidad, setGravamenEntidad] = useState(inicial?.gravamenEntidad || "");
  const [estadoEscritura, setEstadoEscritura] = useState(inicial?.estadoEscritura || "");
  const [proindivisoPorcentaje, setProindivisoPorcentaje] = useState(inicial?.proindivisoPorcentaje ?? "");
  const [anioConstruccion, setAnioConstruccion] = useState(inicial?.anioConstruccion ?? "");
  const [amoblado, setAmoblado] = useState(inicial?.amoblado ?? false);
  const [depositoCuartoUtil, setDepositoCuartoUtil] = useState(inicial?.depositoCuartoUtil ?? false);
  const [balconTerraza, setBalconTerraza] = useState(inicial?.balconTerraza ?? false);
  const [zonasComunes, setZonasComunes] = useState(inicial?.zonasComunes || []);
  ...
  const toggleZonaComun = (z) => setZonasComunes((prev) => (prev.includes(z) ? prev.filter((x) => x !== z) : [...prev, z]));
  ...
  const esApartamento = categoria === "apartamento";
  ...
  const guardar = () => {
    ...
    } else if (esApartamento) {
      const faltantes = [];
      if (areaM2 === "") faltantes.push("Área (m²)");
      if (habitaciones === "") faltantes.push("Habitaciones");
      if (banos === "") faltantes.push("Baños");
      if (parqueaderos === "") faltantes.push("Parqueaderos");
      if (!piso.trim()) faltantes.push("Piso / nivel");
      if (!estrato) faltantes.push("Estrato");
      if (administracion === "") faltantes.push("Administración mensual");
      if (!libreGravamen && !gravamenEntidad.trim()) faltantes.push("Entidad del gravamen");
      if (!estadoEscritura) faltantes.push("Estado de escritura");
      if (estadoEscritura === "proindiviso" && !proindivisoPorcentaje) faltantes.push("Porcentaje del proindiviso");
      if (faltantes.length > 0) {
        setCamposError(`Completa estos campos obligatorios: ${faltantes.join(", ")}.`);
        return;
      }
      detallesFinal = construirDetallesApartamento({
        areaM2, habitaciones, banos, parqueaderos, piso, estrato, administracion,
        libreGravamen, gravamenEntidad, estadoEscritura, proindivisoPorcentaje, anioConstruccion, amoblado, depositoCuartoUtil, balconTerraza, zonasComunes,
      });
    }
    onGuardar({
      nombre: nombre.trim(),
      ...
      ...(esVehiculoOMoto ? {
        ...
        mantenimientosDetalle: categoria === "vehiculo" && mantenimientosAlDia ? mantenimientosDetalle.trim() : "",
        soatEstado,
      ...
      ...(esApartamento ? {
        ...
        libreGravamen,
        gravamenEntidad: libreGravamen ? "" : gravamenEntidad.trim(),
        estadoEscritura,
        proindivisoPorcentaje: estadoEscritura === "proindiviso" ? Number(proindivisoPorcentaje) : null,
        anioConstruccion: anioConstruccion === "" ? null : Number(anioConstruccion),
        amoblado,
        depositoCuartoUtil,
        balconTerraza,
        zonasComunes,
      ...
      <div style={styles.formWrap}>
        ...
        {esVehiculoOMoto ? (
          <>
            ...
            {categoria === "vehiculo" && (
              <>
                ...
                {mantenimientosAlDia && (
                  <input style={styles.input} placeholder="Detalle (opcional) — ej: se entrega con historial de mantenimiento" value={mantenimientosDetalle} onChange={(e) => setMantenimientosDetalle(e.target.value)} />
                )}
              </>
            )}
          ...
          <>
            ...
            <div style={styles.smallLabel}>ÁREA (M²)</div>
            <input style={styles.input} type="number" placeholder="Ej: 80" value={areaM2} onChange={(e) => setAreaM2(e.target.value)} />

            <div style={styles.smallLabel}>HABITACIONES</div>
            <input style={styles.input} type="number" min="0" placeholder="Ej: 3" value={habitaciones} onChange={(e) => setHabitaciones(e.target.value)} />

            <div style={styles.smallLabel}>BAÑOS</div>
            <input style={styles.input} type="number" min="0" placeholder="Ej: 2" value={banos} onChange={(e) => setBanos(e.target.value)} />

            <div style={styles.smallLabel}>PARQUEADEROS</div>
            <input style={styles.input} type="number" min="0" placeholder="0 si no tiene" value={parqueaderos} onChange={(e) => setParqueaderos(e.target.value)} />

            <div style={styles.smallLabel}>PISO / NIVEL</div>
            <input style={styles.input} placeholder="Ej: 4, o PH" value={piso} onChange={(e) => setPiso(e.target.value)} />

            <div style={styles.smallLabel}>ESTRATO</div>
            <select style={styles.input} value={estrato} onChange={(e) => setEstrato(e.target.value)}>
              <option value="">Selecciona…</option>
              {[1, 2, 3, 4, 5, 6].map((n) => (
            ...
            <div style={styles.smallLabel}>ADMINISTRACIÓN MENSUAL</div>
            <PrecioInput style={styles.input} placeholder="0 si no tiene" value={administracion} onChange={setAdministracion} />

            <label style={styles.checkboxRow}>
              <input type="checkbox" checked={libreGravamen} onChange={(e) => setLibreGravamen(e.target.checked)} />
              Libre de gravamen
            </label>
            {!libreGravamen && (
              <input style={styles.input} placeholder="¿Con quién? (ej: Bancolombia)" value={gravamenEntidad} onChange={(e) => setGravamenEntidad(e.target.value)} />
            )}

            <div style={styles.smallLabel}>ESTADO DE ESCRITURA</div>
            <select style={styles.input} value={estadoEscritura} onChange={(e) => setEstadoEscritura(e.target.value)}>
              <option value="">Selecciona…</option>
              <option value="completa">Al 100%</option>
              <option value="proindiviso">Proindiviso</option>
              <option value="en_proceso">En proceso / trámite</option>
            </select>
            {estadoEscritura === "proindiviso" && (
              <input style={{ ...styles.input, marginTop: 8 }} type="number" min="1" max="99" placeholder="¿A qué porcentaje? (ej: 50)" value={proindivisoPorcentaje} onChange={(e) => setProindivisoPorcentaje(e.target.value)} />
            )}
            {(estadoEscritura === "proindiviso" || estadoEscritura === "en_proceso") && (
              <div style={styles.warnBox}>⚠️ No aplica para crédito hipotecario — el banco exige escritura individual y completa. Se agrega automáticamente al texto de publicación.</div>
            )}

            {camposError && <div style={styles.errorText}>{camposError}</div>}

            <div style={styles.sectionDivider} />
            <div style={styles.eyebrow}>CARACTERÍSTICAS OPCIONALES</div>
            ...
            <label style={styles.checkboxRow}>
              <input type="checkbox" checked={depositoCuartoUtil} onChange={(e) => setDepositoCuartoUtil(e.target.checked)} />
              Depósito / cuarto útil
            </label>

            <label style={styles.checkboxRow}>
              <input type="checkbox" checked={balconTerraza} onChange={(e) => setBalconTerraza(e.target.checked)} />
              Balcón / terraza
            </label>

            <div style={styles.smallLabel}>AÑO DE CONSTRUCCIÓN</div>
            <input style={styles.input} type="number" placeholder="Ej: 2015 (opcional)" value={anioConstruccion} onChange={(e) => setAnioConstruccion(e.target.value)} />

            <div style={styles.smallLabel}>ZONAS COMUNES</div>
            <div style={styles.canalesGrid}>
              {ZONAS_COMUNES_SUGERIDAS.map((z) => {
                const activo = zonasComunes.includes(z);
                return (
                  <button key={z} style={{ ...styles.canalChip, ...(activo ? styles.canalChipActive : {}) }} onClick={() => toggleZonaComun(z)}>
                    {z} {activo && <Check size={12} />}
                  </button>
                );
              })}
            </div>
...
const styles = {
  ...
  searchRow: { display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 14 },
  filtrosBtn: { position: "relative", flexShrink: 0, width: 44, height: 44, borderRadius: 12, border: "1px solid #332C25", background: "#1E1A17", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#B0A89B" },
  filtrosBadgeDot: { position: "absolute", top: 8, right: 8, width: 8, height: 8, borderRadius: 4, background: "#E1521B", border: "1.5px solid #1E1A17" },
  filtrosPanel: { background: "#1E1A17", border: "1px solid #302A24", borderRadius: 14, padding: 16, marginBottom: 16, display: "flex", flexDirection: "column", gap: 12, boxShadow: "0 4px 16px rgba(0,0,0,0.3)" },
  filtrosGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 },
  ...
  cardDetalles: { fontSize: 12.5, color: "#8B8377", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: 1.45 },
